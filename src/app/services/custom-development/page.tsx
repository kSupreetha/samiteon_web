    ...
    <main className="min-h-screen pt-24 text-slate-900 dark:text-slate-100">
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-slate-900 via-orange-950 to-slate-900 px-6 py-24 text-white">
        ...
        <div className="relative mx-auto max-w-7xl">
          <p className="mb-3 text-xs font-semibold uppercase tracking-widest text-orange-400">{c.services}</p>
          ...
          <div className="mt-10 flex flex-wrap gap-4">
            <Link href="/contact" className="rounded-lg bg-orange-600 px-8 py-3 font-semibold text-white shadow-lg transition hover:bg-orange-500">{c.inquireNow}</Link>
      ...
      <div className="relative h-72 w-full sm:h-96 lg:h-[28rem]">
        ...
        <div className="absolute bottom-0 left-0 right-0 px-6 pb-10">
          <p className="text-xs font-semibold uppercase tracking-widest text-orange-300">{p.imageLabel}</p>
      ...
      <section className="w-full bg-white px-6 py-14 dark:bg-slate-950">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-8 grid gap-6 sm:grid-cols-2">
            {[
              { title: p.card1Title, body: p.card1Desc, icon: "M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" },
              { title: p.card2Title, body: p.card2Desc, icon: "M13 10V3L4 14h7v7l9-11h-7z" },
            ].map(({ title, body, icon }) => (
        ...
        <div className="mx-auto max-w-7xl grid gap-10 lg:grid-cols-2 lg:items-center">
          <div>
            ...
            <div className="mt-6 space-y-4">
              {[
                { title: p.endToEndTitle, body: p.endToEndDesc },
                { title: p.cuttingEdgeTitle, body: p.cuttingEdgeDesc },
              ].map(({ title, body }) => (
      ...
      <section className="w-full border-t border-slate-200 bg-white px-6 py-16 dark:border-slate-800 dark:bg-slate-950">
        <div className="mx-auto max-w-7xl grid gap-10 lg:grid-cols-2 lg:items-start">
          ...
          <div>
            ...
            <p className="mb-6 text-base leading-relaxed text-slate-600 dark:text-slate-300">{p.sec3Desc}</p>
            <div className="space-y-4">
              {[
                { title: p.mobileTitle, body: p.mobileDesc },
                { title: p.webAppTitle, body: p.webAppDesc },
                { title: p.softwareTitle, body: p.softwareDesc },
              ].map(({ title, body }) => (
      ...
      <div className="relative h-64 w-full sm:h-80">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center px-6 text-center">
          ...
          <p className="mt-4 max-w-xl text-lg text-orange-100">{p.ctaDesc}</p>
          <Link href="/contact" className="mt-8 rounded-lg bg-white px-10 py-4 font-bold text-orange-700 shadow-lg transition hover:bg-orange-50">{c.inquire}</Link>
        </div>
      </div>
